...
void LoggingServer::subscribe(Subscriber *pSub) {
	if (nullptr == pSub) {
...
void LoggingServer::unsubscribe(Subscriber *pSub) {
	if (nullptr == pSub) {
...
void LoggingServer::notify(Event::t_Ptr ptr) {
	for (auto s : _Subs) {
		s->update(ptr);
	}
}

int LoggingServer::onIdle() {
	while (!isEmpty()) {
		notify(pop());
	}
	return _iSleepTime;
